...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!emails || !Array.isArray(emails) || emails.length === 0) {
      return NextResponse.json({ error: 'Missing or invalid emails' }, { status: 400 })
    }

    // Normalize to lowercase and deduplicate within the batch
    const lowercasedEmails = Array.from(new Set(emails.map((e: string) => e.toLowerCase())))
    ...
    if (newEmails.length === 0) {
      return NextResponse.json(
        {
          message: 'All provided emails already exist in the whitelist or are duplicates.',
          count: 0,
        },
        { status: 200 }
      )
