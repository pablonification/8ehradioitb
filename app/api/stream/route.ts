...
const icy = icyImport as unknown as IcyClient
...
export async function GET(request: Request): Promise<Response> {
  ...
  return new Promise((resolve: (response: Response) => void) => {
    ...
    icy
      .get(RADIO_URL, (res: IncomingMessage) => {
        // res is a Node stream (IncomingMessage) with audio data in the body
        const pass = new PassThrough()
        res.pipe(pass)

        // Build response once headers received
        const response = new NextResponse(Readable.toWeb(pass) as unknown as BodyInit, {
          status: 200,
          headers: {
