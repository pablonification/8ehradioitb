...
export async function POST(req: Request) {
  ...
  const { title, coverImage, addCoverImage } = (await req.json()) as {
    title?: string
    coverImage?: string
  ...
  if (config) {
    ...
    config = await prisma.playerConfig.create({
      data: { title: title || '', coverImage, coverImages: coverImage ? [coverImage] : [] },
    })
  }
  return NextResponse.json(config)
}
...
export async function DELETE(req: Request) {
  ...
  if (url === '/8eh.png')
    return NextResponse.json({ error: 'Cannot remove default image' }, { status: 400 })
