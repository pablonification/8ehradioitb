...
export async function buildKruDatabaseRowsForExport({
  ...
  const resolveWithCache = async (key) => {
    if (typeof resolveFileUrl !== "function" || !key) return null;

    if (!fileUrlCache.has(key)) {
      const task = Promise.resolve(resolveFileUrl(key)).catch(() => null);
      fileUrlCache.set(key, task);
    }
    return fileUrlCache.get(key);
  };

  const rowTasks = profiles.map(async (profile, index) => {
    const biodata = normalizeBiodata(profile.biodata);
    const row = {
      No: index + 1,
      "User ID": profile.user?.id || "",
    ...
    const fieldTasks = fieldDefinitions.map(async (field) => {
      const value = biodata[field.key];
      const columnKey = `[${field.key}] ${field.label}`;

      if (field.fieldType !== "file" || typeof resolveFileUrl !== "function") {
        row[columnKey] = stringifyProfileValue(value);
        return;
      }
      ...
      const urls = await Promise.all(keys.map((key) => resolveWithCache(key)));
      row[columnKey] = urls.filter(Boolean).join(" | ");
    });
