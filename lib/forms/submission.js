...
function buildQuestionColumnMap(questions, sections = []) {
  ...
  normalizedQuestions.forEach((question, index) => {
    ...
    let columnName = `q.${label}`;

    if ((labelCounts.get(label) || 0) > 1) {
      const sectionTitle = sectionTitleById.get(question?.sectionId) || "Bagian";
      columnName = `q.${label} (${sectionTitle})`;
    }

    const seenCount = (columnCounts.get(columnName) || 0) + 1;
    columnCounts.set(columnName, seenCount);
    if (seenCount > 1) {
      columnName = `${columnName} #${seenCount}`;
    }

    map.set(answerKey, columnName);
  });
...
export async function flattenSubmissionRows({
  ...
  const rows = [];
  const normalizedQuestions = Array.isArray(questions) ? questions : [];
  const questionColumnMap = buildQuestionColumnMap(
    normalizedQuestions,
    sections,
  );
  ...
  for (const submission of submissions) {
    ...
    for (const question of normalizedQuestions) {
      ...
      const questionColumn =
        questionColumnMap.get(answerKey) || `q.${question.label || answerKey}`;

      if (question.fieldType === "file_upload" && Array.isArray(answerValue)) {
        const links = await resolveFileLinks(answerValue, buildFileUrl);
        row[questionColumn] = links;
        continue;
      }

      row[questionColumn] = serializeAnswerForExport(answerValue);
    }
