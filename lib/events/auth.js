...
export const ADMIN_ROLES = ["DEVELOPER"];
...
export async function canPerformEventAction(userId, eventId, action) {
  ...
  try {
    ...
    if (role && hasAnyRole(role, ADMIN_ROLES)) {
      return { allowed: true, reason: "admin_bypass" };
    }

    const organizerMembership = await prisma.eventOrganizer.findUnique({
      where: {
        eventId_userId: {
          eventId,
          userId,
    ...
    if (organizerMembership) {
      return { allowed: true, reason: "organizer_membership" };
    }
